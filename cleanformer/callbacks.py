...
class LogMetricsCallback(Callback):
    ...
    def on_train_batch_end(
        ...
        pl_module.log("Train/Accuracy", F.accuracy(out["logits"], out["tgt"]), on_step=True, on_epoch=True)
    ...
    def on_validation_batch_end(
        ...
        pl_module.log("Validation/Accuracy_epoch", F.accuracy(out["logits"], out["tgt"]), on_epoch=True)
...
class LogBLEUCallback(Callback):
    ...
    def on_validation_batch_end(
        ...
        losses, _ = pl_module.step(src, tgt_r, tgt)  # -> (N, L)
        tgt_hat = pl_module.infer(src, tgt_r)
        self.cache['tgt'] = self.cache.get('tgt', list()) + out['tgt'].cpu().tolist()
        self.cache['tgt_hat'] = self.cache.get('tgt_hat', list()) + tgt_hat.cpu().tolist()
        self.cache['losses'] = self.cache.get('losses', list()) + losses.sum(dim=1).cpu().tolist()  # (N, L) -> (N,)
    ...
    def on_validation_epoch_end(self, trainer: Trainer, pl_module: Transformer) -> None:
        ...
        self.logger.log_text(f"Validation/samples", columns=["prediction", "answer", "losses"],  # noqa
    ...
    def on_test_epoch_end(self, trainer: Trainer, pl_module: Transformer) -> None:
        predictions = self.tokenizer.decode_batch(self.cache['tgt_hat'])  # (N, L) -> (N,) = list
        answers = self.tokenizer.decode_batch(self.cache['tgt'])  # (N, L) -> (N,) = list
        self.logger.log_text(f"Test/samples", columns=["prediction", "answer"],  # noqa
